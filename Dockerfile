...
ARG WEBSERVICE_ENVIRONMENT
ENV WEBSERVICE_ENVIRONMENT=${WEBSERVICE_ENVIRONMENT:-homologation}
...
RUN cd /tmp/webservice \
    ...
    && make
    # && cd /tmp/webservice && make install \
...
#    && /tmp/apache-setup-for-dockerfile.sh $WEBSERVICE_ENVIRONMENT \
